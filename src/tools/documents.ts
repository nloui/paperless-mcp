...
export function registerDocumentTools(server, api) {
  server.tool(
    ...
    async (args, extra) => {
      ...
      return toContent(await api.bulkEditDocuments(documents, method, parameters));
    }
  );
  ...
  server.tool(
    ...
    async (args, extra) => {
      ...
      return toContent(await api.postDocument(file, metadata));
    }
  );
  ...
  server.tool(
    ...
    async (args, extra) => {
      if (!api) throw new Error("Please configure API connection first");
      return toContent(await api.getDocument(args.id));
    }
  );
  ...
  server.tool(
    ...
    async (args, extra) => {
      if (!api) throw new Error("Please configure API connection first");
      return toContent(await api.searchDocuments(args.query, args.page, args.page_size));
    }
  );
